// DlgGraphicTasks.cpp : implementation file
//
...
// CDlgGraphicTasks dialog

IMPLEMENT_DYNAMIC(CDlgGraphicTasks, CDialog)

CDlgGraphicTasks::CDlgGraphicTasks(CWnd* pParent /*=NULL*/)	: CDialog(CDlgGraphicTasks::IDD, pParent)
{
	m_pDoc = NULL;
...
CDlgGraphicTasks::~CDlgGraphicTasks()
{
	if (!m_hIcon)
	{
		delete m_hIcon;
...
void CDlgGraphicTasks::InitialUpdate(CRemoteCombinedDoc *pDoc, bool bShowWindow)
{
	...
	if (bShowWindow)
	{
		if (!m_bTimerActive)
		{
			SetTimer(UTIMER_DLG_GRAPHIC_TASKS, 1000, 0);
			m_bTimerActive = true;
		}
	}
}

void CDlgGraphicTasks::ShowControls(bool bShow)
{
	int iShow;
...
void CDlgGraphicTasks::PostCommandView(int iCommand)
{
...
void CDlgGraphicTasks::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CDlgGraphicTasks, CDialog)
	ON_WM_SIZE()
	ON_WM_SIZING()
	ON_WM_LBUTTONDBLCLK()
	ON_WM_TIMER()
	...
	ON_LBN_SELCHANGE(IDC_LIST_GRAPHIC_COMPUTERS, &CDlgGraphicTasks::OnLbnSelchangeListGraphicComputers)
	ON_LBN_SELCHANGE(IDC_LIST_GRAPHIC_PROJECTS, &CDlgGraphicTasks::OnLbnSelchangeListGraphicProjects)
	...
	ON_BN_CLICKED(IDC_BUTTON_UPDATE, &CDlgGraphicTasks::OnBnClickedButtonUpdate)
	ON_EN_CHANGE(IDC_EDIT_GRAPHIC_LINE, &CDlgGraphicTasks::OnEnChangeEditGraphicLine)
	ON_BN_CLICKED(IDC_CHECK_GRAPHIC_PROJECTS_SAME, &CDlgGraphicTasks::OnBnClickedCheckGraphicProjectsSame)
	ON_BN_CLICKED(IDC_CHECK_GRAPH_PERIOD, &CDlgGraphicTasks::OnBnClickedCheckGraphPeriod)
	ON_EN_CHANGE(IDC_EDIT_GRAPHIC_PERIOD, &CDlgGraphicTasks::OnEnChangeEditGraphicPeriod)
	ON_BN_CLICKED(IDC_CHECK_GRAPHIC_BLOCK, &CDlgGraphicTasks::OnBnClickedCheckGraphicBlock)
	ON_BN_CLICKED(IDC_CHECK_GRAPHIC_SINGLE_MULTIPLE, &CDlgGraphicTasks::OnBnClickedCheckGraphicSingleMultiple)
	ON_BN_CLICKED(IDC_CHECK_GRAPHIC_AVERAGE, &CDlgGraphicTasks::OnBnClickedCheckGraphicAverage)
	ON_BN_CLICKED(IDC_CHECK_GRAPHIC_EXTENDED2, &CDlgGraphicTasks::OnBnClickedCheckGraphicExtended2)
	ON_BN_CLICKED(IDC_CHECK_GRAPHIC_CPU, &CDlgGraphicTasks::OnBnClickedCheckGraphicCpu)
	ON_BN_CLICKED(IDC_CHECK_GRAPHIC_GPU, &CDlgGraphicTasks::OnBnClickedCheckGraphicGpu)

	ON_BN_CLICKED(ID_HELP, &CDlgGraphicTasks::OnBnClickedHelp)
END_MESSAGE_MAP()


// CDlgGraphicTasks message handlers

BOOL CDlgGraphicTasks::OnInitDialog()
{
	int iCheck;
...
// Translation
	...
	this->SetWindowText(gszTranslation[PosDialogGraphicTasksTitle]);
...
// Translation
	...
	m_iLineThickness = theApp.GetProfileInt(registrySectionGraphicTasks, registryGraphicLineThickness, 2);
	if (m_iLineThickness < 1) m_iLineThickness = 1;
	CString sTxt;
	sTxt.Format("%d",m_iLineThickness);
	m_editLineThickness.SetWindowText(sTxt);

	iCheck = theApp.GetProfileInt(registrySectionGraphicTasks, registryGraphicCombineProject, 0);
	m_bCheckCombineProjects = (iCheck != 0);
	m_checkCombineProject.SetCheck(m_bCheckCombineProjects);

	iCheck = theApp.GetProfileInt(registrySectionGraphicTasks, registryBlockMode, 1);
	m_bBlockMode = (iCheck != 0);
	m_checkModeBlock.SetCheck(m_bBlockMode);

	iCheck = theApp.GetProfileInt(registrySectionGraphicTasks, registrySingleMultiple, 1);
	...
	iCheck = theApp.GetProfileInt(registrySectionGraphicTasks, registryAverage, 0);
	m_bCheckAverage = (iCheck != 0);
	m_checkAverage.SetCheck(m_bCheckAverage);

	iCheck = theApp.GetProfileInt(registrySectionGraphicTasks, registryGraphicPeriod, 0);
	...
	m_iPeriod = theApp.GetProfileInt(registrySectionGraphicTasks, registryGraphicPeriodValue, 1);
	if (m_iPeriod < 1) m_iPeriod = 1;
	sTxt.Format("%d",m_iPeriod);
	m_editPeriod.SetWindowText(sTxt);

	iCheck = theApp.GetProfileInt(registrySectionGraphicTasks, registryCpu, 1);
	m_checkCpu.SetCheck(iCheck);
	m_bCpu = (iCheck != 0);
	iCheck = theApp.GetProfileInt(registrySectionGraphicTasks, registryGpu, 1);
	...
	int left = theApp.GetProfileInt(registrySectionWindowPosition, registryGraphicTasksPosLeft, 0);
	int top  = theApp.GetProfileInt(registrySectionWindowPosition, registryGraphicTasksPosTop, 0);
	int width = theApp.GetProfileInt(registrySectionWindowPosition, registryGraphicTasksPosRight, 800) - left;
	int height = theApp.GetProfileInt(registrySectionWindowPosition, registryGraphicTasksPosBottom, 540) - top;
...
void CDlgGraphicTasks::SafeWindowPlacement()
{
	WINDOWPLACEMENT wp;
	GetWindowPlacement(&wp);
	theApp.WriteProfileInt(registrySectionWindowPosition, registryGraphicTasksPosLeft, wp.rcNormalPosition.left);
	theApp.WriteProfileInt(registrySectionWindowPosition, registryGraphicTasksPosTop, wp.rcNormalPosition.top);
	theApp.WriteProfileInt(registrySectionWindowPosition, registryGraphicTasksPosRight, wp.rcNormalPosition.right);
	theApp.WriteProfileInt(registrySectionWindowPosition, registryGraphicTasksPosBottom, wp.rcNormalPosition.bottom);
}

LRESULT CDlgGraphicTasks::OnCloseGraphic(WPARAM parm1, LPARAM parm2)
{
	m_bClosing = true;
	m_bClosingReady = false;
	return 0;
...
BOOL CDlgGraphicTasks::PreTranslateMessage(MSG* pMsg)
{
...
void CDlgGraphicTasks::ResetProjects()
{
}

void CDlgGraphicTasks::CheckOneGraph()
{
	int iSelectionCount;
...
void CDlgGraphicTasks::OnTimer(UINT_PTR nIDEvent)
{
	if (m_bShowWindow)
	{
	...
	if (!this->IsWindowVisible())
	{
		if (m_bTimerActive)
		{
			KillTimer(UTIMER_DLG_GRAPHIC_TASKS);
			m_bTimerActive = false;
		}
		return;
	}
...
//	m_sLog.m_pFrom = "CDlgGraphicTasks::OnTimerDlg";
	...
	else
	{
		if (m_bClosing)
		{
			if (!m_bClosingReady)
			{
				...
				PostCommandView(UWM_RESTART_GRAPHIC_TASKS_READY);
//				EndDialog(0);
//				KillTimer(UTIMER_DLG_GRAPHIC_TASKS);
				return;
			}
	...
	if (!m_window.IsPainting())
	{
		if (m_bStartUpdateStatistics && !m_bClosing && !m_bClosingReady)
		{
			...
			if (!m_window.IsPainting())
			{
				...
				{
					::PostThreadMessage(m_lComputer.at(iCount)->m_ThreadRpcId,UWM_MSG_THREAD_START_RPC_GRAPHIC_TASKS_P, (WPARAM) this->m_hWnd,(LPARAM) iPeriod);
					std::deque<CHistoryRpc*> *pHistoryRpc;
					pHistoryRpc = m_pDoc->m_pHistoryView->GetComputerList();
					::PostThreadMessage(m_lComputer.at(iCount)->m_ThreadRpcId,UWM_MSG_THREAD_START_RPC_GRAPHIC_TASKS_L, (WPARAM) hWnd,(LPARAM) pHistoryRpc->at(iCount));			// history pointers
					::PostThreadMessage(m_lComputer.at(iCount)->m_ThreadRpcId,UWM_MSG_THREAD_START_RPC_GRAPHIC_TASKS, (WPARAM) this->m_hWnd,(LPARAM) m_lComputer.at(iCount)->m_plHistoryProjectTasks);
					m_bThreadBusy = true;
				}
			}
		}
...
void CDlgGraphicTasks::OnSize(UINT nType, int cx, int cy )
{
	SetWindowPosition(cx, cy, nType);
//	this->Invalidate(TRUE);
}

void CDlgGraphicTasks::OnSizing(UINT fwSide, LPRECT pRect)
{
...
void CDlgGraphicTasks::SetWindowPosition(LONG lWidth, LONG lHeight, UINT nType)
{
...
LRESULT CDlgGraphicTasks::OnInitialGraphic(WPARAM parm1, LPARAM parm2)
{
	char *pc;
...
LRESULT CDlgGraphicTasks::OnGraphicsTasksReady(WPARAM parm1, LPARAM parm2)
{
	int		iPos;
	int iResult;
	...
	for (int iCountThread = 0; iCountThread < m_iNrThreadRpc; iCountThread++)
	{
		...
		{
			plHistoryProjectTasks = m_lComputer.at(iCountThread)->m_plHistoryProjectTasks;
			break;
		}
	}

	if (plHistoryProjectTasks)
	{
		for (iPos = 0; iPos < (int) plHistoryProjectTasks->size(); iPos++)
		{
			pHistoryProjectTasksItem = plHistoryProjectTasks->at(iPos);
			pcProjectName = pHistoryProjectTasksItem->m_pcProject;
...
LRESULT CDlgGraphicTasks::OnColorChanged(WPARAM parm1, LPARAM parm2)
{
	UpdateGraphic();
...
void CDlgGraphicTasks::OnShowWindow(BOOL bShow, UINT nStatus)
{
...
void CDlgGraphicTasks::OnEnChangeEditGraphicLine()
{
	CString sTxt;
	...
	theApp.WriteProfileInt(registrySectionGraphicTasks, registryGraphicLineThickness, m_iLineThickness);

	UpdateGraphic();
}

void CDlgGraphicTasks::OnBnClickedButtonUpdate()
{
...
void CDlgGraphicTasks::UpdateGraphic()
{
...
void CDlgGraphicTasks::AddProject(CString sComputer, CString sProject, COLORREF color)
{
	int		iPos;
	CHistoryProjectTasksItem	*pHistoryProjectTasksItem;
	CString		sProjectUrl;
	...
	for (int iCountComputer = 0; iCountComputer < m_iNrThreadRpc; iCountComputer++)
	{
		...
		{
			// computer found
			for (iPos = 0; iPos < (int) m_lComputer.at(iCountComputer)->m_plHistoryProjectTasks->size(); iPos++)
			{
				pHistoryProjectTasksItem = m_lComputer.at(iCountComputer)->m_plHistoryProjectTasks->at(iPos);
				if (sProject == pHistoryProjectTasksItem->m_pcProject)
				{
					m_window.AddProject(sComputer,pHistoryProjectTasksItem,color);
				}
			}
		}
	}
...
void CDlgGraphicTasks::OnLbnSelchangeListGraphicComputers()
{
	CheckOneGraph();
	UpdateGraphic();
}

void CDlgGraphicTasks::OnLbnSelchangeListGraphicProjects()
{
	UpdateGraphic();
}

void CDlgGraphicTasks::OnBnClickedCheckGraphicProjectsSame()
{
	int iCheck;
	...
	theApp.WriteProfileInt(registrySectionGraphicTasks, registryGraphicCombineProject, iCheck);
...
/*
void CDlgGraphicTasks::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	if (m_lComputer.size() == 0) return;

	for (int iPosComputer = 0; iPosComputer < m_lComputer.size(); iPosComputer++)
	{
		if (m_lComputer.at(iPosComputer)->m_plHistoryProjectTasks->size() > 0)
		{
			if (bFirst)
			{
				theApp.m_pDlgProperties->SetWindowText("Tasks Graph statistics");
				theApp.m_pDlgProperties->SendMessage(UWM_MSG_DLG_PROPERTIES_SETUP,2,(LPARAM) registryPropertiesTasksGraph);	// 2 columns
				bFirst = false;
			}
			for (int iPosProject; iPosProject < m_lComputer.at(iPosComputer)->m_plHistoryProjectTasks->size(); iPosProject++)
			{	
				sColumn0 = "Computer";
				sColumn1 = m_lComputer.at(iPosComputer)->m_pcThreadRpcId;
				theApp.m_pDlgProperties->SendMessage(UWM_MSG_DLG_PROPERTIES_ADD_ROW, (WPARAM) &sColumn0, (LPARAM) &sColumn1);

				sColumn0 = "Project";
				sColumn1 = m_lComputer.at(iPosComputer)->m_plHistoryProjectTasks->at(iPosProject)->m_pcProject;
				theApp.m_pDlgProperties->SendMessage(UWM_MSG_DLG_PROPERTIES_ADD_ROW, (WPARAM) &sColumn0, (LPARAM) &sColumn1);
				sColumn0 = "";
				sColumn1 = "";
				theApp.m_pDlgProperties->SendMessage(UWM_MSG_DLG_PROPERTIES_ADD_ROW, (WPARAM) &sColumn0, (LPARAM) &sColumn1);

				GetAverage(m_lComputer.at(iPosComputer)->m_plHistoryProjectTasks, &iDay, &iWeek); 
				sColumn0 = "Last day total";
				sColumn1.Format("%d", iDay);
				theApp.m_pDlgProperties->SendMessage(UWM_MSG_DLG_PROPERTIES_ADD_ROW, (WPARAM) &sColumn0, (LPARAM) &sColumn1);
				sColumn0 = "Last week total";
				sColumn1.Format("%d", iWeek);
				theApp.m_pDlgProperties->SendMessage(UWM_MSG_DLG_PROPERTIES_ADD_ROW, (WPARAM) &sColumn0, (LPARAM) &sColumn1);
			}
		}
	}
...
void CDlgGraphicTasks::OnLButtonDblClk(UINT nFlags, CPoint point)
{
...
void CDlgGraphicTasks::OnBnClickedCheckGraphPeriod()
{
	int		iCheck;
	...
	theApp.WriteProfileInt(registrySectionGraphicTasks, registryGraphicPeriod, iCheck);
	m_bStartUpdateStatistics = true;	// make sure the RPC is called again
}


void CDlgGraphicTasks::OnEnChangeEditGraphicPeriod()
{
	CString sTxt;
	...
	theApp.WriteProfileInt(registrySectionGraphicTasks, registryGraphicPeriodValue, m_iPeriod);
	m_bStartUpdateStatistics = true;	// make sure the RPC is called again
}


void CDlgGraphicTasks::OnBnClickedCheckGraphicBlock()
{
	int		iCheck;
	...
	theApp.WriteProfileInt(registrySectionGraphicTasks, registryBlockMode, iCheck);
	UpdateGraphic();
//	m_bStartUpdateStatistics = true;	// make sure the RPC is called again
}


void CDlgGraphicTasks::OnBnClickedCheckGraphicSingleMultiple()
{
	int		iCheck;

	iCheck = m_checkSingleMultiple.GetCheck();
	theApp.WriteProfileInt(registrySectionGraphicTasks, registrySingleMultiple, iCheck);
...
void CDlgGraphicTasks::OnBnClickedCheckGraphicAverage()
{
	int		iCheck;

	iCheck = m_checkAverage.GetCheck();
	m_bCheckAverage = (iCheck != 0);
	theApp.WriteProfileInt(registrySectionGraphicTasks, registryAverage, iCheck);
	UpdateGraphic();
}


void CDlgGraphicTasks::OnBnClickedCheckGraphicExtended2()
{
	int		iCheck;

	iCheck = m_checkExpanded.GetCheck();
	theApp.WriteProfileInt(registrySectionGraphicTasks, registryExpanded, iCheck);

	PostCommandView(UWM_RESTART_GRAPHIC_TASKS);

	m_checkExpanded.EnableWindow(FALSE);
}


void CDlgGraphicTasks::OnBnClickedCheckGraphicCpu()
{
	int iCheck;
	...
	theApp.WriteProfileInt(registrySectionGraphicTasks, registryCpu, iCheck);
	UpdateGraphic();
}


void CDlgGraphicTasks::OnBnClickedCheckGraphicGpu()
{
	int iCheck;
	...
	theApp.WriteProfileInt(registrySectionGraphicTasks, registryGpu, iCheck);
	UpdateGraphic();
}

BOOL CDlgGraphicTasks::OnHelpInfo( HELPINFO* HelpInfo)
{
	OnBnClickedHelp();
	return TRUE;
}

void CDlgGraphicTasks::OnBnClickedHelp()
{
	CString strLink;
	strLink = "https://efmer.com/boinctasks-graphs#tasks";
