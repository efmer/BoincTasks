...
LOG_FLAGS_V6::LOG_FLAGS_V6() {
    init();
}

void LOG_FLAGS_V6::init() {
    memset(this, 0, sizeof(LOG_FLAGS_V6));
...
//
int LOG_FLAGS_V6::parse(XML_PARSER& xp) {
    while (!xp.get_tag()) {
        if (!xp.is_tag) {
            continue;
        }
...
int LOG_FLAGS_V6::write(MIOFILE& out) {
    out.printf(
        "    <log_flags>\n"
...
int EXCLUDE_GPU_V6::parse(XML_PARSER& xp) {
...
//
int CONFIG_V6::parse_options(XML_PARSER& xp) {
    string s;
    int n, retval;
...
int CONFIG_V6::parse(XML_PARSER& xp, LOG_FLAGS_V6& log_flags) {
    while (!xp.get_tag()) {
        if (!xp.is_tag) {
            continue;
        }
...
int CONFIG_V6::write(MIOFILE& out, LOG_FLAGS_V6& log_flags) {
    int j;
    unsigned int i;
