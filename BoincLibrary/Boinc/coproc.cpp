...
void COPROC_NVIDIA::fake(
   ...
   cuda_version = 2020;
   have_cuda = true;
   strcpy_s(prop.name, "Fake NVIDIA GPU");
...
void COPROC_ATI::fake(double ram, double avail_ram, int n) {
    strcpy_s(type, GPU_TYPE_ATI);
    strcpy_s(version, "1.4.3");
    strcpy_s(name, "foobar");
    count = n;
    available_ram = avail_ram;
    have_cal = true;
    memset(&attribs, 0, sizeof(attribs));
